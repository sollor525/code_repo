...
- **`POST /api/packet/export` saves to a directory** — `output_dir` defaults to the process CWD, `filename` defaults to `packet_<timestamp>.pcap`; returns `{filename,path,size}`. Same behavior as `/pcap/save` and `/pcapedit/modify`; **all three route through `pcap_generator::write_pcap_to_dir(bytes, dir, name, prefix)`** (default dir = CWD, default name = `<prefix>_<ts>.pcap`, basename-only, `.pcap` enforced, `\\?\` stripped) — change save behavior there, not in three places. The packet.html UI uses export-to-dir; its old blob `下载 PCAP 文件` button was removed (same Tauri user-gesture/`blob:` issue as pcapgen). `POST /api/packet/download` still returns the bytes as an in-memory attachment (fine for plain server mode; the UI no longer uses it). Note: packet.html's built-in TCP/UDP examples were malformed (odd-length hex) and were replaced with well-formed even-length frames.
