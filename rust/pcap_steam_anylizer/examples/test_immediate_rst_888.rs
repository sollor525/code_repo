...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 测试用例1：SYN包后立即跟随RST-888
    println!("测试用例1：SYN包后立即跟随RST-888报文");
    ...
    // 测试用例2：SYN包后先有其他数据包，然后才是RST-888
    println!("\n测试用例2：SYN包后有其他数据包，然后才是RST-888报文");
    ...
    // 测试用例3：SYN包后没有RST-888报文
    println!("\n测试用例3：SYN包后没有RST-888报文（正常连接）");
    ...
    let mut streams_with_immediate_rst_888 = 0;
    let mut streams_without_immediate_rst_888 = 0;
    ...
    for stream in manager.get_all_streams() {
        ...
        if stream.connection.handshake.client_syn {
            if stream.has_immediate_rst_888_after_syn {
                streams_with_immediate_rst_888 += 1;
                println!("  会话 {}: SYN包后立即跟随RST-888报文", stream.flow_key.to_string());
                println!("    SYN包后的数据包数: {}", stream.packets_since_syn);
            } else {
                streams_without_immediate_rst_888 += 1;
                println!("  会话 {}: SYN包后没有立即跟随RST-888报文", stream.flow_key.to_string());
                println!("    SYN包后的数据包数: {}", stream.packets_since_syn);
            }
        }
    }
    ...
    println!("  SYN包后立即跟随RST-888的流数: {}", streams_with_immediate_rst_888);
    println!("  SYN包后没有立即跟随RST-888的流数: {}", streams_without_immediate_rst_888);
