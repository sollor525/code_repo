...
use tls_ja4_core::tls::client_hello::*;
...
/// 基准测试：原始TLS解析（无缓存）
fn bench_original_parsing(data_list: &[Vec<u8>], iterations: usize) -> Duration {
...
/// 基准测试：带缓存的TLS解析
fn bench_cached_parsing(data_list: &[Vec<u8>], iterations: usize) -> Duration {
    // 清理缓存确保公平测试
...
mod tests {
    ...
    fn test_cache_performance_improvement() {
        // 创建测试数据
        let data_list = create_test_data_variants(100); // 100个不同的TLS数据包

        let iterations = 1000; // 每个数据包解析1000次

        // 测试原始解析性能
        let original_time = bench_original_parsing(&data_list, iterations);

        // 测试缓存解析性能
        let cached_time = bench_cached_parsing(&data_list, iterations);

        // 计算性能提升
        let improvement_ratio = original_time.as_nanos() as f64 / cached_time.as_nanos() as f64;
        let improvement_percent = (improvement_ratio - 1.0) * 100.0;

        println!("Performance Test Results:");
        println!("Original parsing time: {:?}", original_time);
        println!("Cached parsing time: {:?}", cached_time);
        println!("Performance improvement: {:.2}x ({:.2}%)", improvement_ratio, improvement_percent);

        // 验证缓存确实提升了性能
        // 在理想情况下，缓存应该显著提升性能
        // 但由于Rust的优化，可能提升有限，所以我们设置一个相对宽松的标准
    ...
    fn test_cache_hit_performance() {
        ...
        // 验证缓存命中时间很短
        assert!(cached_time.as_nanos() < 100_000_000, // 100ms
                "Cache hits should be very fast");
    }

    #[test]
    fn test_cache_memory_efficiency() {
        let data_list = create_test_data_variants(1000); // 1000个不同的数据包
    ...
    fn test_concurrent_cache_performance() {
        ...
        let data_list = create_test_data_variants(100);
        let thread_count = 4;
