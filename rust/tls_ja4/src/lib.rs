...
pub fn extract_alpn_from_client_hello(client_hello_data: &[u8]) -> Option<String> {
    // 使用手动解析，因为tls-parser可能无法正确解析某些Client Hello
    extract_alpn_manual(client_hello_data)
}
...
pub fn save_fingerprints_to_file(
    ...
    for (session_key, session) in sessions {
        ...
        let src_ip = client_parts[0].to_string();
        let src_port = client_parts[1].parse::<u16>().unwrap_or(0);
        let dst_ip = server_parts[0].to_string();
        let dst_port = server_parts[1].parse::<u16>().unwrap_or(0);
